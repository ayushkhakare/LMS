...
function Offer() {
  ...
  const getBoxColor = (time, expired) => {
    if (expired) return "bg-gray-500 text-white";
  ...
  return (
    <div className="bg-[#0d1721] flex flex-col items-center justify-center p-6 min-h-100">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6 w-full max-w-6xl">
        {offers.map((offer) => (
          <div
            key={offer.id}
            className={`relative p-6 rounded-xl shadow-2xl transform hover:scale-110 transition duration-300 border-4 border-red-500 ${getBoxColor(offer.expiresIn, offer.expired)} animate-pulse`}
          >
            ...
            {offer.expired ? (
              ...
              <p className="font-semibold mt-2 animate-pulse">
